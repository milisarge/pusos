...
#define IRC_USER "pusos"
#define IRC_NICK "pusos"
#define IRC_CHAN "#milisarge"
...
status_t irc_client::console_input_line(const char *line, bool &exit) {
    ...
    char buf[256];
    snprintf(buf, sizeof(buf), "PRIVMSG #milisarge :%s\r\n", line);
...
status_t irc_client::handshake() {
    ...
    // send USER and NICK
    char buf[128];
    snprintf(buf, sizeof(buf), "USER %s host server :mahmutov\r\n", IRC_USER);
    status_t err = tcp_write(sock_, buf, strlen(buf));
    if (err < 0) {
