...
def hb_checker():
  ...
  while(1):
    for p in list(peers):
      ...
      if p not in peers_hb: peers_hb[p] = hb_wait # ilkleme
      print(p,diff.total_seconds(), peers_hb[p], peers_load[p])
      ...
      try:
        if diff.total_seconds() < peers_hb[p]:
          ...
          peers_load[p] = 7
          peers_hb[p] = 1.2 # global değerin atanması
        if diff.total_seconds() > peers_load[p]:
          ...
          peers_load[p] += diff.total_seconds() * 8
